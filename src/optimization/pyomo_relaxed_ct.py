...
class WWTPPlantOptimizer:
    ...
    def _build_unit_submodel(self, unit_name, unit_type):
        ...
        # Define bounds for scaled variables, crucial for McCormick relaxations.
        # A range of [-10, 10] is a reasonable assumption for standardized variables.
        scaled_var_bounds = (-1, 1)
        
        b.X = pyo.Var(b.M)
        b.X_s = pyo.Var(b.M, bounds=scaled_var_bounds)
        b.Y_s = pyo.Var(b.K, bounds=scaled_var_bounds)

        # Auxiliary variables for bilinear terms w = x*y
        # For x,y in [-10, 10], w is in [-100, 100]
        w_bounds = (scaled_var_bounds[0] * scaled_var_bounds[1], scaled_var_bounds[1]**2)

        # w_yx represents Y_s[k] * X_s[m]
        b.w_yx = pyo.Var(b.K, b.M, bounds=w_bounds)
        # w_xx represents X_s[m1] * X_s[m2] for pairs in L
        ...
        # McCormick envelopes for w_yx = Y_s * X_s
        x_L, x_U = scaled_var_bounds
        
        @b.Constraint(b.K, b.M)
        def mccormick_yx_1(b, k, m):
            return b.w_yx[k, m] >= x_L * b.X_s[m] + x_L * b.Y_s[k] - x_L * x_L

        @b.Constraint(b.K, b.M)
        def mccormick_yx_2(b, k, m):
            return b.w_yx[k, m] >= x_U * b.X_s[m] + x_U * b.Y_s[k] - x_U * x_U

        @b.Constraint(b.K, b.M)
        def mccormick_yx_3(b, k, m):
            return b.w_yx[k, m] <= x_U * b.X_s[m] + x_L * b.Y_s[k] - x_U * x_L
        
        @b.Constraint(b.K, b.M)
        def mccormick_yx_4(b, k, m):
            return b.w_yx[k, m] <= x_L * b.X_s[m] + x_U * b.Y_s[k] - x_L * x_U

        # McCormick envelopes for w_xx = X_s[m1] * X_s[m2]
        @b.Constraint(b.L)
        def mccormick_xx_1(b, m1, m2):
            return b.w_xx[m1, m2] >= x_L * b.X_s[m2] + x_L * b.X_s[m1] - x_L * x_L

        @b.Constraint(b.L)
        def mccormick_xx_2(b, m1, m2):
            return b.w_xx[m1, m2] >= x_U * b.X_s[m2] + x_U * b.X_s[m1] - x_U * x_U

        @b.Constraint(b.L)
        def mccormick_xx_3(b, m1, m2):
            return b.w_xx[m1, m2] <= x_U * b.X_s[m2] + x_L * b.X_s[m1] - x_U * x_L

        @b.Constraint(b.L)
        def mccormick_xx_4(b, m1, m2):
            return b.w_xx[m1, m2] <= x_L * b.X_s[m2] + x_U * b.X_s[m1] - x_L * x_U

        @b.Constraint(b.K)
        def CBREM_model_rule(b, K_name):
            k_idx = k_map[K_name]
            # Original bilinear terms are replaced by their auxiliary variables (w_yx and w_xx)
            term1 = b.Y_s[K_name]
