...
from naming import CONCENTRATION_SUFFIX, canonical_base_name, legacy_identifier, normalize_stream_column, strip_prefix_and_units
...
def _normalized_component_key(raw_comp: str, prefix: str) -> str:
    """
    Convert a legacy ASM token or composite token into the descriptive base
    name used by the prediction outputs, and append the stage prefix.
    Example: S_NH4 + CSTR1 -> ammonia_nitrogen_CSTR1
             BOD + Effluent -> biochemical_oxygen_demand_Effluent
    """
    base = canonical_base_name(raw_comp) or strip_prefix_and_units(raw_comp)
...
def generate_treatment_train_plots(sim_type, stages, stage_names, predicted_df):
    ...
    def get_predictions_for_component(df, comp_id, stages_map, stage_name_list):
        base_name = canonical_base_name(comp_id) or strip_prefix_and_units(comp_id)
        predictions = []
...
def run_single_cstr_simulation(inputs: dict, sim_type: str, enable_plotting: bool):
    ...
    try:
        ...
        for idx, comp_id in enumerate(effluent_stream.components.IDs):
            sim_key = _normalized_component_key(comp_id, 'Effluent')
            simulated_effluent[sim_key] = effluent_stream.conc[idx]
        composites = _get_simulated_composites(effluent_stream)
        for key, val in composites.items():
            sim_key = _normalized_component_key(key, 'Effluent')
            simulated_effluent[sim_key] = val
    except Exception as e:
    ...
    try:
        ...
        for prefix, stream in streams_to_report.items():
            for idx, comp_id in enumerate(stream.components.IDs):
                sim_key = _normalized_component_key(comp_id, prefix)
                simulated_results[sim_key] = stream.conc[idx]
            composites = _get_simulated_composites(stream)
            for key, val in composites.items():
                 sim_key = _normalized_component_key(key, prefix)
                 simulated_results[sim_key] = val
    except Exception as e:
        print(f"\nAN ERROR OCCURRED during the QSDsan simulation: {e}"); traceback.print_exc(); return None, None
    finally:
        qs.main_flowsheet.clear()
...
def run_cstr_and_clarifier_validation(sim_type: str, enable_plotting: bool):
    optimization_filepath = os.path.join('data', 'results', 'optimization', 'clarifier', 'optimization_results.xlsx')
    initial_cond_filepath = os.path.join('data', 'config', 'simulation_training_config.xlsx')
    try:
...
def main():
    ...
    while True:
        ...
        if choice in ['1', 'cstr']: 
            ...
            run_cstr_and_clarifier_validation(sim_type='clarifier', enable_plotting=enable_plotting)
            break
