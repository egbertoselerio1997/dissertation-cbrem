...
solver_type = input(f"Enter solver type {valid_solvers} [default: solve]: ").lower().strip() or 'solve'

valid_optimizers = ['adam', 'sgd', 'rmsprop', 'adamw']
optimizer_type = input(f"Enter optimizer type {valid_optimizers} [default: adamw]: ").lower().strip() or 'adamw'
...
def plot_parity(Y_true, Y_pred, y_cols):
    ...
    plt.tight_layout()
    save_path = os.path.join(IMG_DIR, 'parity_plot.png')
...
def main():
    ...
    for fold, (tr_idx, te_idx) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS)):
        ...
        Y_pred_s = predict_pytorch(model, X_te_s, Z_te_s)
        Y_pred = np.exp(sc_y.inverse_transform(Y_pred_s))
        
        # Calculate stats
        for i, col in enumerate(y_cols):
            mse = mean_squared_error(Y_te.iloc[:, i], Y_pred[:, i])
            r2 = r2_score(Y_te.iloc[:, i], Y_pred[:, i])
            all_stats.append({'Fold': fold+1, 'Variable': col, 'MSE': mse, 'RMSE': np.sqrt(mse), 'R2': r2})

    cv_df = pd.DataFrame(all_stats)
    cv_summary = cv_df.groupby('Variable').agg(['mean', 'std'])
