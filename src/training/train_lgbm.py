...
# !pip install pandas numpy scikit-learn lightgbm openpyxl joblib tqdm
...
def train_lightgbm_model(X_train: np.ndarray, Y_train: np.ndarray):
    """
    Initializes and trains a multi-output LightGBM model.
    """
...
def main():
    """Main function to orchestrate the entire workflow for LightGBM."""
    ...
    print("\n   - Applying natural log transformation to Y for training.")
    Y_log = np.log(Y)
    ...
    for fold, (train_index, test_index) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS, desc="Cross-Validation")):
        ...
        model_fold = train_lightgbm_model(X_train_s, Y_log_train_s)
    ...
    final_model = train_lightgbm_model(X_s_full, Y_log_s_full)
