...
# --- Training Logic ---
def train_ann_model(X_train, Y_train, n_epochs, batch_size, learning_rate, verbose=False):
...
def main():
    ...
    for fold, (tr_idx, te_idx) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS)):
        ...
        model = train_ann_model(X_tr_s, Y_tr_s, NUM_EPOCHS, BATCH_SIZE, LEARNING_RATE)
    ...
    final_model = train_ann_model(X_s, Y_s, NUM_EPOCHS, BATCH_SIZE, LEARNING_RATE, verbose=True)
