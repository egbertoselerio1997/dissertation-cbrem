...
def get_hyperparameters(X_s, Y_s):
    ...
    if file_exists:
        try:
            ...
            required_cols = ['learning_rate', 'n_estimators', 'max_depth', 'min_child_weight', 'gamma', 'subsample', 'colsample_bytree']
            if all(col in df_params.columns for col in required_cols) and not df_params.empty:
                ...
                params['min_child_weight'] = int(df_params['min_child_weight'].iloc[0])
                params['gamma'] = float(df_params['gamma'].iloc[0])
                params['subsample'] = float(df_params['subsample'].iloc[0])
                params['colsample_bytree'] = float(df_params['colsample_bytree'].iloc[0])
    ...
    def objective(trial):
        ...
        model = MultiOutputRegressor(estimator=xgb_reg, n_jobs=-1)
...
def main():
    ...
    for fold, (tr_idx, te_idx) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS)):
        ...
        # XGBoost Model with fetched hyperparameters
    ...
    final_model = MultiOutputRegressor(estimator=xgb_reg, n_jobs=-1)
