...
MACHINE_LEARNING_MODEL = 'xgboost'
...
def train_xgboost_model(X_train: np.ndarray, Y_train: np.ndarray):
    """
    Initializes and trains a multi-output XGBoost model.
    """
    xgb_reg = XGBRegressor(
        ...
        verbosity=0  # Set to 0 to reduce log spam
    )

    multi_output_model = MultiOutputRegressor(estimator=xgb_reg, n_jobs=-1)
...
def main():
    """Main function to orchestrate the entire workflow for XGBoost."""
    ...
    for fold, (train_index, test_index) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS, desc="Cross-Validation")):
        ...
        Y_test = Y.iloc[test_index] # Original scale for final evaluation

        # Fit scalers ONLY on the training data of this fold
        ...
        model = train_xgboost_model(X_train_s, Y_log_train_s)

        # Predict on the test set of this fold and inverse transform
    ...
    final_model = train_xgboost_model(X_s_full, Y_log_s_full)
    ...
    with pd.ExcelWriter(OUTPUT_FILE_PATH, engine='xlsxwriter') as writer:
        # Save aggregated cross-validation stats (NEW SHEET)
        aggregated_stats.to_excel(writer, sheet_name='cross_val_statistics')

        # Save data based on the final model trained on the full dataset
        ...
        # Save empty dataframes to maintain the original report structure
