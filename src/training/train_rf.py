...
def train_random_forest_model(X_train: np.ndarray, Y_train: np.ndarray):
    """
    Initializes and trains a Random Forest model.
    """
...
def main():
    """Main function to orchestrate the entire workflow for Random Forest."""
    ...
    for fold, (train_index, test_index) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS, desc="Cross-Validation")):
        ...
        model_fold = train_random_forest_model(X_train_s, Y_log_train_s)
    ...
    final_model = train_random_forest_model(X_s_full, Y_log_s_full)
