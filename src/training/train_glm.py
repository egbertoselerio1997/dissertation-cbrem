...
# --- Data Loading and Preparation (Identical to the original script) ---
...
# --- GLM Training Function ---
def train_glm_model(X_train: np.ndarray, Y_train: np.ndarray):
    """
    Initializes and trains a multi-output Generalized Linear Model (GLM).
    This uses LinearRegression because the data passed has been log-transformed.
    """
...
def main():
    """Main function to orchestrate the entire workflow for GLM."""
    ...
    if (Y <= 0).any().any():
        print("   - Warning: Y contains zero or negative values. Using log1p (log(1+Y)) transformation.")
        Y_log = np.log1p(Y)
    ...
    for fold, (train_index, test_index) in enumerate(tqdm(kf.split(X, Y_log), total=N_SPLITS, desc="Cross-Validation")):
        ...
        model_fold = train_glm_model(X_train_s, Y_log_train_s)

        Y_pred_log_s = model_fold.predict(X_test_s)
        Y_pred_log = y_scaler.inverse_transform(Y_pred_log_s)
        Y_pred_test = inverse_transform(Y_pred_log)
    ...
    final_model = train_glm_model(X_s_full, Y_log_s_full)
    ...
    model_bundle = {
        ...
        'inverse_transform_func': 'expm1' if inverse_transform == np.expm1 else 'exp'
    }
    ...
    Y_pred_full_log_s = final_model.predict(X_s_full)
    Y_pred_full_log = final_y_scaler.inverse_transform(Y_pred_full_log_s)
    Y_pred_full = inverse_transform(Y_pred_full_log)
